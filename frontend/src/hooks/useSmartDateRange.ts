...
export function useSmartDateRange(): SmartDateRangeResult {
  ...
  const result = useMemo(() => {
    ...
    rangeOptions.push({
      label: earliestDate.getFullYear() === latestDate.getFullYear() ? 
    ...
    if (latestDate >= thirtyDaysAgo) {
      rangeOptions.push({
        label: 'Last 30 Days',
        startDate: format(Math.max(thirtyDaysAgo.getTime(), earliestDate.getTime()), 'yyyy-MM-dd'),
        endDate: format(Math.min(now.getTime(), latestDate.getTime()), 'yyyy-MM-dd')
      });
    }
    ...
    if (latestDate >= threeMonthsAgo) {
      rangeOptions.push({
        label: 'Last 3 Months',
        startDate: format(Math.max(threeMonthsAgo.getTime(), earliestDate.getTime()), 'yyyy-MM-dd'),
        endDate: format(Math.min(now.getTime(), latestDate.getTime()), 'yyyy-MM-dd')
      });
    }
    ...
    if (latestDate >= sixMonthsAgo) {
      rangeOptions.push({
        label: 'Last 6 Months',
        startDate: format(Math.max(sixMonthsAgo.getTime(), earliestDate.getTime()), 'yyyy-MM-dd'),
        endDate: format(Math.min(now.getTime(), latestDate.getTime()), 'yyyy-MM-dd')
      });
    }
    ...
    if (latestDate >= twelveMonthsAgo) {
      rangeOptions.push({
        ...
        startDate: format(Math.max(twelveMonthsAgo.getTime(), earliestDate.getTime()), 'yyyy-MM-dd'),
        endDate: format(Math.min(now.getTime(), latestDate.getTime()), 'yyyy-MM-dd')
      });
    }
    ...
    const rangesWithCounts: DateRangeOption[] = rangeOptions.map(range => {
      ...
      const transactionCount = transactions.items.filter((transaction: Transaction) => {
    ...
    if (optimalRange.transactionCount < MIN_TRANSACTION_THRESHOLD && rangesWithCounts.length > 1) {
      const alternativeRange = rangesWithCounts.slice(1).find(range => 
        range.transactionCount >= MIN_TRANSACTION_THRESHOLD
      );
    ...
    return {
      optimalRange,
      availableRanges: rangesWithCounts.filter(range => range.transactionCount > 0)
    };
  }, [transactions]);
