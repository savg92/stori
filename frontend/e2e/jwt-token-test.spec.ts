...
test.describe('JWT Token Extraction for API Testing', () => {
  test('extract JWT token after login and test API', async ({ page }) => {
    ...
    console.log('🔑 JWT Token extracted:', token ? 'Token found!' : 'No token found');
    ...
    console.log('📊 API Response:', {
      ...
      dataPreview: typeof apiResponse.data === 'string' ? apiResponse.data.substring(0, 100) : apiResponse.data
    });
    ...
    if (apiResponse.status === 401) {
      ...
      console.log('⚠️ Unexpected API response:', apiResponse.status, apiResponse.data);
    }
  });
});