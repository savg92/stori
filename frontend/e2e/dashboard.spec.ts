			...
			await route.fulfill({
				...
				body: JSON.stringify({
					...
					categories: [
						...
						{ category: 'Transportation', amount: -200.0, count: 8 },
						{ category: 'Entertainment', amount: -150.75, count: 5 },
						{ category: 'Shopping', amount: -449.5, count: 12 },
					],
					monthly_trend: [
						{ month: '2024-01', income: 5000.0, expenses: -1100.5 },
						{ month: '2024-02', income: 5000.0, expenses: -1250.5 },
					],
				}),
			});
		});
				...
				await route.fulfill({
					...
					body: JSON.stringify({
						...
						categories: [
							...
							{ category: 'Transportation', amount: -400.0, count: 4 },
						],
						monthly_trend: [
							{ month: '2024-02', income: 2500.0, expenses: -650.25 },
						],
					}),
				});
			} else {
			...
			if (url.searchParams.get('category')) {
				await route.fulfill({
					...
					body: JSON.stringify({
						...
						total_income: 0.0,
						net_amount: -450.25,
						categories: [{ category: 'Food', amount: -450.25, count: 15 }],
						monthly_trend: [
							{ month: '2024-01', income: 0.0, expenses: -200.25 },
							{ month: '2024-02', income: 0.0, expenses: -250.0 },
						],
					}),
				});
			} else {
	...
	test('should refresh data', async ({ page }) => {
		...
		await page.route('**/api/expenses/summary*', async (route) => {
			...
			await route.fulfill({
				...
				contentType: 'application/json',
				body: JSON.stringify({
					total_expenses: callCount === 1 ? 1250.5 : 1350.75,
					total_income: 5000.0,
					net_amount: callCount === 1 ? 3749.5 : 3649.25,
					categories: [
						{
							category: 'Food',
							amount: callCount === 1 ? -450.25 : -550.5,
							count: 15,
						},
					],
					monthly_trend: [],
