...
test.describe('AI Financial Advisor', () => {
  ...
  test('should display AI chat interface', async ({ page }) => {
    ...
    await expect(page.getByText(/hello! i'm your ai financial advisor/i)).toBeVisible()
  })
  ...
  test('should send message and receive AI response', async ({ page }) => {
    ...
    await page.route('**/api/ai/advice', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          response: "Based on your spending patterns, I notice you're spending quite a bit on food ($450 this month). Consider meal planning to reduce expenses by 20-30%. You're doing well with your savings rate of 75%!",
          suggestions: [
    ...
    await expect(page.getByText('How can I reduce my food expenses?')).toBeVisible()
    
    // Check AI response
    await expect(page.getByText(/based on your spending patterns/i)).toBeVisible()
    ...
    await expect(page.getByText('Set a food budget of $350/month')).toBeVisible()
  })
  ...
  test('should handle multiple conversation turns', async ({ page }) => {
    ...
    await expect(page.getByText('Can you help me create a budget?')).toBeVisible()
    await expect(page.getByText('Yes, please analyze my spending')).toBeVisible()
  })
  ...
  test('should display suggested questions', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /how can i save more money/i })).toBeVisible()
    await expect(page.getByRole('button', { name: /analyze my spending habits/i })).toBeVisible()
    await expect(page.getByRole('button', { name: /help me create a budget/i })).toBeVisible()
  })
  ...
  test('should use suggested question', async ({ page }) => {
    ...
    await page.route('**/api/ai/advice', async route => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          response: "Looking at your expenses, you're already doing great with a 75% savings rate! To save even more, consider reducing your food expenses by meal planning.",
          suggestions: [
    ...
    await page.getByRole('button', { name: /how can i save more money/i }).click()
    
    // Check question appears in chat
    await expect(page.getByText('How can I save more money?')).toBeVisible()
    
    // Check AI response
    await expect(page.getByText(/already doing great with a 75% savings rate/i)).toBeVisible()
  })
  ...
  test('should display loading state during AI response', async ({ page }) => {
    ...
    // Wait for response
    await expect(page.getByText("Here's my advice...")).toBeVisible({ timeout: 3000 })
